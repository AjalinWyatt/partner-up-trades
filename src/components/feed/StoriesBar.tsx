...
const StoriesBar = ({ groups, myAvatarUrl, myName, onAddStory, onOpenStory }: StoriesBarProps) => {
  return (
    <div className="border-b border-border px-4 py-3">
      <div className="flex gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <button onClick={onAddStory} className="flex w-[72px] shrink-0 flex-col items-center gap-1.5 text-center">
        ...
        {groups.map((group, index) => (
          <button key={group.userId} onClick={() => onOpenStory(index)} className="flex w-[72px] shrink-0 flex-col items-center gap-1.5 text-center">
            <div
              className={cn(
                "rounded-full p-[2px]",
