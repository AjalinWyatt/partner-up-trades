...
const SignUp = () => {
  ...
  const BETA_KEY = "MYOBxTWapp";
  const [betaUnlocked, setBetaUnlocked] = useState(
    () => localStorage.getItem("tw:beta-unlocked:v5") === "1"
  );
  ...
  const handleBetaSubmit = (e: React.FormEvent) => {
    ...
    if (betaInput.trim() === BETA_KEY) {
      localStorage.removeItem("tw:beta-unlocked"); localStorage.removeItem("tw:beta-unlocked:v2"); localStorage.removeItem("tw:beta-unlocked:v3"); localStorage.removeItem("tw:beta-unlocked:v4");
      localStorage.setItem("tw:beta-unlocked:v5", "1");
